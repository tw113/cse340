...
// Get Image Information from images table by invId
function getImagesByInvId($invId) {
  $db = phpmotorsConnect();
  $sql = "SELECT imgId, imgPath, imgName, imgDate, invId, imgPrimary FROM images WHERE invId = :invId AND imgName LIKE '%-tn%'";
  $stmt = $db->prepare($sql);
  $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
  $stmt->execute();
  $imageArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
